...
# fig.show()
show_simple_scatter(dates, y, "New Covid Cases", "Date", "New Cases", dtick=1)
...
# subset_days = days[400:450]
# peaks, _ = find_peaks(subset_days, height=) # TODO how do we pick prominence??
# show_simple_scatter(days, y, "New Covid Cases", "Date", "New Cases", dtick=1)

# show_simple_scatter(days, y , "Lomb-Scargle", "days", "New Cases",)
...
show_simple_scatter(1 / frequency, power, "Lomb-Scargle Periodogram", "Period (days)", "Lomb-Scargle Power")

#sorted_indices = np.argsort(power)[::-1]
#sorted_freqs = frequency[sorted_indices]
#sorted_power = power[sorted_indices]
probs = np.array([ls.false_alarm_probability(p) for p in power])

non_nan_indices = np.where(~np.isnan(probs))

freqs = frequency[non_nan_indices]
power = power[non_nan_indices]
probs = probs[non_nan_indices]

possible_signal_indices = np.where(probs < 0.01)

possible_frequencies = freqs[possible_signal_indices]
possible_power = power[possible_signal_indices]

show_simple_scatter(1 / possible_frequencies, possible_power, "Lomb-Scargle", "period", "power")
